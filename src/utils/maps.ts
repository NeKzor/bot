...
export class Portal2Map {
  name: string;
  alias: string;
  type: number;
  bestTimeId: number | null;
  chapterId: number;
  bestPortalsId: number | null;
  order_id: number;

  get isOfficial() {
    return (
      (this.type === Portal2MapType.SinglePlayer ||
        this.type === Portal2MapType.Cooperative) &&
  ...
  constructor(
    ...
    this.order_id = Portal2Map.totalCount++;
  }

  static searchByName(name: string) {
    return CampaignMaps.find((map) => map.name === name);
  }
  static searchById(id: number) {
    return CampaignMaps.find((map) =>
      map.bestTimeId === id || map.bestPortalsId == id
    );
  }
  static singlePlayerMaps() {
    return CampaignMaps.filter((map) =>
      map.type === Portal2MapType.SinglePlayer
    );
  }
  static cooperativeMaps() {
    return CampaignMaps.filter((map) =>
      map.type === Portal2MapType.Cooperative
    );
  }
}
